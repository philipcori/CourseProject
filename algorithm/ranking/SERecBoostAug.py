...
class SERecBoostAug(SocialRecommender):
    def __init__(self,conf,trainingSet=None,testSet=None,relation=None,fold='[1]'):
        super(SERecBoostAug, self).__init__(conf,trainingSet,testSet,relation,fold)

    def initModel(self):
        super(SERecBoostAug, self).initModel()
        self.lam_theta = 0.01  # From Paper
        self.lam_beta = 0.01  # From Paper
        self.lam_y = 0.01
        self.init_mu = 0.01
        self.a = 1.0  # alpha 1 
        self.b = 99.0  # alpha 2 
        ...
        for user in self.data.trainSet_u:
            for item in self.data.trainSet_u[user]:
                ...
                val.append(1)
        self.X = csr_matrix(
            (np.array(val),(np.array(row),np.array(col))),
    ...
    def _update(self, X):
        ...
        print("Weighted Rating Matrix Factorization")
        self._init_factors(iterations=8)  #8 is good
        self._update_expo(X, n_users)

        print("Expectation Maximization to Estimate Latent Factors")
        for i in xrange(self.maxIter):
            print 'ITERATION #%d' % i

            print("Update user and item collaborative factors")
            self._update_factors(X, XT)

            # print self.mu
            print("Update exposure prior")
            self._update_expo(X, n_users)

            self.ranking_performance(i)


    def _init_factors(self, iterations):
        iteration = 0
        while iteration < iterations:
            self.loss = 0
            YtY = self.beta.T.dot(self.beta)
            for user in self.data.user:
                ...
                for item in self.data.trainSet_u[user]:
                    ...
                    H[iid]+=10*r_ui
                    P_u[iid]=1
                    error = (P_u[iid]-self.theta[uid].dot(self.beta[iid]))
                    self.loss+=pow(error,2)
                #sparse matrix
                C_u = coo_matrix((val,(pos,pos)),shape=(self.num_items,self.num_items))
                A = (YtY + np.dot(self.beta.T,C_u.dot(self.beta))+self.regU*np.eye(self.embed_size))
                self.theta[uid] = np.dot(np.linalg.inv(A),(self.beta.T*H).dot(P_u))

            XtX = self.theta.T.dot(self.theta)
            for item in self.data.item:
                ...
                A = (XtX + np.dot(self.theta.T,C_i.dot(self.theta))+self.regU*np.eye(self.embed_size))
                self.beta[iid]=np.dot(np.linalg.inv(A), (self.theta.T*H).dot(P_i))
    ...
    def _update_factors(self, X, XT):
        ...
        # recompute_factors(X, X_old, Y, lam, lam_y, mu, n_jobs, batch_size=1000)

        self.theta = recompute_factors(
            X=self.beta, 
            X_old=self.theta, 
            Y=X,
            lam=self.lam_theta / self.lam_y,
            lam_y=self.lam_y,
            mu=self.mu,
            n_jobs=self.n_jobs,
        ...
        self.beta = recompute_factors(
            X=self.theta, 
            X_old=self.beta, 
            Y=XT,
            lam= self.lam_beta / self.lam_y,
            lam_y=self.lam_y,
            mu=self.mu,
            n_jobs=self.n_jobs,
            batch_size=self.batch_size
